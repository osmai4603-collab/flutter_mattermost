...
/// حمولة ويبهوك إنشاء تشغيل (WebhookOnCreationPayload):
/// جميع حقول PlaybookRun إضافة إلى روابط القناة والتفاصيل.
class WebhookOnCreationPayloadEntity extends PlaybookRunEntity {
  final String? channel_url;
  final String? details_url;

  const WebhookOnCreationPayloadEntity({
    super.id,
    super.name,
    super.summary,
    super.is_active,
  ...
  @override
  WebhookOnCreationPayloadEntity copyWith({
    String? id,
    String? name,
    String? summary,
    bool? is_active,
    ...
    return WebhookOnCreationPayloadEntity(
      id: id ?? this.id,
      name: name ?? this.name,
